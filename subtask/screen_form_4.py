...
combined_df["Hypertension"] = combined_df["Comorbidities"].apply(
    lambda x: 1 if "高血压" in str(x) else 0
)
combined_df["Diabetes"] = combined_df["Comorbidities"].apply(
    lambda x: 1 if "糖尿病" in str(x) else 0
)
...
combined_df.to_excel("4总_75岁及以上.xlsx", index=False)
print(
    "已根据 Comorbidities 列在 4总_75岁及以上.xlsx 中新增了 Hypertension 和 Diabetes 两列。"
)

# 只保留“Pathological Diagnosis”中“DA”这一项
combined_df = combined_df[
    combined_df["Pathological Diagnosis"].str.contains("DA", na=False)
]
...
print(
    "已将 Surgical procedure 列添加到 4总_75岁及以上_手动补全.xlsx 文件中，并根据 住院号 进行匹配。"
)
