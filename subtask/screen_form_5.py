...
df_combined["In-hospital Death"] = (
    df_combined["住院号"]
    .map(death_lookup)
    .combine_first(df_combined["In-hospital Death"])
)
df_combined["ASA Score"] = (
    df_combined["住院号"].map(asa_lookup).combine_first(df_combined["ASA Score"])
)
df_combined["OS"] = (
    df_combined["住院号"].map(os_lookup).combine_first(df_combined["OS"])
)
df_combined["Survival Status"] = (
    df_combined["住院号"]
    .map(survival_lookup)
    .combine_first(df_combined["Survival Status"])
)
# 保存修改后的文件
df_combined.to_excel("4总_75岁及以上.xlsx", index=False)
print(
...
df_combined["Chemotherapy situation"] = df_combined[
